...
## 02.10.22 Laura Sudupe , git @lsudupe
...
levels(a) <- list(HSC_PSC  = "Ery/Mk prog.", 
                  HSC_PSC = "Neutro prog.",
                  HSC_PSC = "Mono prog.",
                  HSC_PSC = "Gran/Mono prog.",
                  HSC_PSC = "LMPPs",
                  IC = "large pre-B.",
                  HSC_PSC = "Mk prog.",
                  HSC_PSC = "Erythroblasts",
                  HSC_PSC = "Eo/Baso prog.",
                  IC = "Monocytes",
                  HSC_PSC = "Ery prog.",
                  IC = "pro-B",
                  IC = "T cells",
...
single_cell_bonemarrow@meta.data[["groups"]] <- a
saveRDS(single_cell_bonemarrow, "./objects/sc/single_cell_bonemarrow.rds")
...
pdf(file.path("./results/clusters/single_cell/",filename = "groups_singlecell.pdf"))
...
predictions_seurat <- SingleR(test=seurat.sce, assay.type.test=1, 
                          ref=single.sce, labels=single.sce$groups)
...
saveRDS(b, "./objects/sp/integrated/integrated.seurat_predictions_group.rds")
saveRDS(predictions_seurat, "./objects/sp/integrated/seurat_predictions_group.rds")

pdf(file.path("./results/singleR/",filename = "groups_singlecell_seurat.pdf"))
...
b@active.ident <- b@meta.data[["pred"]]
saveRDS(b, "./objects/sp/integrated/integrated.harmony_predictions_group.rds")
saveRDS(predictions_harmony, "./objects/sp/integrated/harmony_predictions_group.rds")

pdf(file.path("./results/singleR/",filename = "groups_singlecell_harmony.pdf"))
print(SpatialDimPlot(b, combine = FALSE,label=TRUE,repel=TRUE ,label.size = 0.9))
dev.off()
